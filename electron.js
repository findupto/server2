...
function create(){const w=new BrowserWindow({width:1440,height:900,minWidth:1000,minHeight:650,backgroundColor:'#f4f6f9',webPreferences:{contextIsolation:true,preload:path.join(__dirname,'electron-preload.js')}});w.loadURL('http://127.0.0.1:4173')}
const bootstraps=['production-upgrades.js','smart-premium-bootstrap.js','ai-understanding-bootstrap.js','ai-status-bootstrap.js','ai-premium-bootstrap.js','ai-language-enterprise-bootstrap.js','enterprise-hardening-bootstrap.js','enterprise-intelligence-bootstrap.js','enterprise-backup-bootstrap.js','financial-hardening-bootstrap.js','ai-model-policy.js','premium-bootstrap.js','ai-bootstrap.js','ai-monitor-bootstrap.js','ai-autonomous-bootstrap.js','ai-agent-bootstrap.js','ai-vision-bootstrap.js','ai-realtime-bootstrap.js','ai-supervisor-bootstrap.js','realtime-sync-bootstrap.js','api-security-bootstrap.js','local-ai-bootstrap.js'];
app.whenReady().then(()=>{const migration=spawnSync(process.execPath,['migrate.js'],{cwd:__dirname,env:{...process.env,PORT:'4173'},stdio:'ignore'});if(migration.status!==0)throw new Error('POS database migration failed');server=spawn(process.execPath,[...bootstraps.flatMap(x=>['-r',path.join(__dirname,x)]),path.join(__dirname,'server.js')],{cwd:__dirname,env:{...process.env,PORT:'4173'},stdio:'ignore'});setTimeout(create,1800)});
